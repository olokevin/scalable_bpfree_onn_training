...
def relative_l2_error(u_pred,u_true):
    l2_diff = np.linalg.norm(u_pred-u_true)
    
    l2_true = np.linalg.norm(u_true)
    
    relative_error = l2_diff / l2_true
...
def val_func_init_from_config_ADFD(config):
    ...
    def val_func(model, dataset, *args):
        x, u = dataset.eval()
        u_pred = to_numpy(x[:, :1] * x[:, 1:] * (x[:, :1] - 1) * (x[:, 1:] - 1) * model(x))
        # mse_error = ((u_pred - u) ** 2).mean()

        # return mse_error
    
        rel_l2_error = relative_l2_error(u_pred,u)
        return rel_l2_error
