...
def loss_func_init_from_config(config):
    ...
    def loss_func(model, dataset, inputs=None, return_loss_reduction='mean'):
        ...
        f_model = lambda X: -torch.sin(torch.pi*X[:,0:1]) + (1-X[:,0:1]**2)*X[:,1:2]*model(X)
        
        
        
        x_interior = x[0:num_interior,...]
        # x_initial = x[num_interior:num_interior+num_boundary,...]
        # x_boundary_min = x[num_interior+num_boundary:num_interior+2*num_boundary,...]
        ...
        pde_loss = (u_t + u_interior*u_x - (0.01/torch.pi)*u_xx).pow(2).mean()
        # u_initial = model(x_initial)
        # initial_loss = (u_initial + torch.sin(torch.pi * x_initial[:,0:1])).pow(2).mean()
        # u_boundary_min = model(x_boundary_min)
        # boundary_loss_min = u_boundary_min.pow(2).mean()
        # u_boundary_max = model(x_boundary_max)
        # boundary_loss_max = u_boundary_max.pow(2).mean()
        loss = pde_loss #+ initial_loss + boundary_loss_min + boundary_loss_max
