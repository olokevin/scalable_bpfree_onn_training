...
class KLLossMixed(torch.nn.modules.loss._Loss):
    """
    description: Knowledge distillation loss function combines hard target loss and soft target loss, [XXXX]
    ...
    T {scalar, Optional} temperature of the softmax function to make the teacher score softer. Default is 6 when accuracy is high, e.g., >95%. Typical value 1~20 [XXXX]
    alpha {scalar, Optional} interpolation between hard and soft target loss. Default set to 0.9 for distillation mode. When hard target loss is very small (alpha=0.9), gets the best results. [XXXX]
